        ...
        with tf.control_dependencies([optimizer_pos]):
            ...
            with tf.control_dependencies([optimizer_neg]):
                ...
                with tf.control_dependencies([optimizer_back]):
                    ...
                    leader_grads = leader_opt.compute_gradients(
                        valid_loss, var_list=unrolled_model.arch_parameters())
        for i, (g, v) in enumerate(leader_grads):
            leader_grads[i] = (
                g - self.learning_rate * tf.divide(train_grads_pos[i]-train_grads_neg[i], 2*R), v)
